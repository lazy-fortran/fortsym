module fortsym
    ...
    use fortsym_connection, only: covariant_diff, covariant_derivative, &
        covariant_divergence, connection_t, connection_create, connection_from_chart, &
    ...
    use fortsym_magnetic, only: magnetic_field_t, magnetic_field, magnetic_upper, &
        ...
        magnetic_chart_flux_form, b_con, b_cov, b_density, b_flux_form, b_con_form, &
        b_density_form, h_cov, h_con, b_fourier, &
    ...
    public :: DIM, chart_t, coords, make_chart, chart_create, chart_create_on_patch, chart_valid, &
        ...
        b_cov, b_density, b_flux_form, b_con_form, b_density_form, h_cov, h_con, b_fourier, &
        ...
        laplace_de_rham, form_from_tensor, &
...
contains
    ...
    !> refuses malformed or over-bounded inputs.
    function rank(expression) result(result)
        type(expr_t), intent(in) :: expression
        type(engine_result_t) :: result
        logical :: ok
        character(:), allocatable :: why
        ...
        if (.not. is_valid(expression)) then
            call report_failure(result, "rank: invalid expression")
            return
        end if
        if (use_default_engine(expression)) then
    ...
    !> Return the exact inverse of a dense matrix represented as nested `List`
    !> expressions. Singular matrices and malformed forms are refused.
    function inv(expression) result(result)
        type(expr_t), intent(in) :: expression
        type(engine_result_t) :: result
        logical :: ok
        character(:), allocatable :: why
        ...
        if (.not. is_valid(expression)) then
            call report_failure(result, "inv: invalid expression")
            return
        end if
        if (use_default_engine(expression)) then
    ...
    !> Return a bounded exact reduced matrix and zero-based pivot columns.
    function rref(expression) result(result)
        type(expr_t), intent(in) :: expression
        type(engine_result_t) :: result
        logical :: ok
        character(:), allocatable :: why
        ...
        if (.not. is_valid(expression)) then
            call report_failure(result, "rref: invalid expression")
            return
        end if
