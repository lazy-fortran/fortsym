...
contains
    ...
    !> Positive infinity as a structural sentinel. It is deliberately not a
    !> real literal: finite numeric evaluation, arithmetic, and real-kernel
    !> emission must reject it until their domain rules are implemented.
    function oo_expr(a) result(e)
        type(arena_t), target, intent(inout) :: a
        type(expr_t)                         :: e
        e = const(a, "oo")
    ...
    !> Modified Bessel function of the first kind I_order(x).
    function fn_besseli_ee(order, x) result(e)
        type(expr_t), intent(in) :: order, x
        type(expr_t)             :: e
        type(expr_t) :: args(2)
        args(1) = order
        args(2) = x
        e = func("besseli", args)
    end function fn_besseli_ee

    function fn_besseli_ie(order, x) result(e)
        integer, intent(in) :: order
        type(expr_t), intent(in) :: x
        type(expr_t)             :: e
