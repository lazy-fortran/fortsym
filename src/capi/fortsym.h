...
int fortsym_matrix_is_diagonal(fortsym_arena *arena,
                               ...
                               char *message, size_t capacity);
/* Three-valued zero-matrix predicate. `verdict` uses enum fortsym_verdict. */
int fortsym_matrix_is_zero_matrix(fortsym_arena *arena,
                                   const fortsym_expr *matrix, int *verdict,
                                   char *message, size_t capacity);
/* Boolean upper-triangular predicate. `verdict` uses enum fortsym_verdict;
 * undecidable entries in the forbidden triangle are treated as nonzero. */
int fortsym_matrix_is_upper(fortsym_arena *arena,
                            const fortsym_expr *matrix, int *verdict,
                            char *message, size_t capacity);
/* Boolean lower-triangular predicate. `verdict` uses enum fortsym_verdict;
 * undecidable entries in the forbidden triangle are treated as nonzero. */
int fortsym_matrix_is_lower(fortsym_arena *arena,
                            const fortsym_expr *matrix, int *verdict,
                            char *message, size_t capacity);
/* Boolean upper/lower-Hessenberg predicates. `verdict` uses enum
 * fortsym_verdict; undecidable forbidden entries are treated as nonzero. */
int fortsym_matrix_is_upper_hessenberg(fortsym_arena *arena,
                                       const fortsym_expr *matrix,
                                       int *verdict, char *message,
                                       size_t capacity);
int fortsym_matrix_is_lower_hessenberg(fortsym_arena *arena,
                                       const fortsym_expr *matrix,
                                       int *verdict, char *message,
                                       size_t capacity);
...
int fortsym_matrix_is_echelon(fortsym_arena *arena,
                              const fortsym_expr *matrix, int *verdict,
                              char *message, size_t capacity);
/* Three-valued Hermitian predicate using structural complex conjugation. */
int fortsym_matrix_is_hermitian(fortsym_arena *arena,
                                const fortsym_expr *matrix, int *verdict,
                                char *message, size_t capacity);
 ...
 * enum fortsym_verdict. */
int fortsym_matrix_is_anti_symmetric(fortsym_arena *arena,
 ...
 * FORTSYM_UNSUPPORTED. */
int fortsym_matrix_rank(fortsym_arena *arena, const fortsym_expr *matrix,
                        fortsym_expr **out, char *message, size_t capacity);
/* Exact inverse of a nonempty square dense matrix represented by nested List
 * expressions. Singular, ragged, and unsupported forms return
 * FORTSYM_UNSUPPORTED. */
int fortsym_matrix_inverse(fortsym_arena *arena, const fortsym_expr *matrix,
                           fortsym_expr **out, char *message, size_t capacity);
/* Transpose a nonempty dense matrix represented by nested List expressions.
...
int fortsym_chart_b_con_form(
    ...
    int orientation, fortsym_expr *out[], char *message, size_t capacity);
int fortsym_chart_b_density_form(
    fortsym_arena *arena, const fortsym_expr *coordinates[],
    const fortsym_expr *position[], const fortsym_expr *input[], size_t degree,
    int orientation, fortsym_expr *out[], char *message, size_t capacity);
...
int fortsym_chart_boozer_residuals(
    ...
    int label_index, fortsym_expr *out[], char *message, size_t capacity);
int fortsym_chart_hamada_residuals(
    fortsym_arena *arena, const fortsym_expr *coordinates[],
    const fortsym_expr *position[], const fortsym_expr *vector[],
    int label_index, fortsym_expr *out[], char *message, size_t capacity);
...
int fortsym_chart_fourier_longitudinal_flux(
    fortsym_arena *arena, const fortsym_expr *coordinates[],
    const fortsym_expr *position[], const fortsym_expr *reluctivity[],
    const fortsym_expr *potential, int component, fortsym_expr **out,
    char *message, size_t capacity);
int fortsym_chart_fourier_transverse_flux(
    fortsym_arena *arena, const fortsym_expr *coordinates[],
    const fortsym_expr *position[], const fortsym_expr *reluctivity[],
...
int fortsym_chart_fourier_longitudinal_residual(
    ...
    fortsym_expr **out, char *message, size_t capacity);
int fortsym_chart_fourier_transverse_residual(
    fortsym_arena *arena, const fortsym_expr *coordinates[],
    const fortsym_expr *position[], const fortsym_expr *reluctivity[],
    const fortsym_expr *potential[], const fortsym_expr *current[],
    const fortsym_expr *mode, fortsym_expr *out[], char *message,
...
int fortsym_spacetime_metric_laplacian(
    ...
    const fortsym_expr *scalar, fortsym_expr **out, char *message,
    size_t capacity);
/* Typed spacetime tensors use the fixed four-coordinate ABI for transport,
 * with inactive lower-dimensional slots returned as zero handles. Input and
 * output component arrays therefore have `4**rank` entries, in
 * first-slot-fastest order. */
int fortsym_spacetime_tensor_raise(
    fortsym_arena *arena, const fortsym_expr *components[], int dimension,
    const fortsym_expr *coordinates[], const int signature[], int orientation,
    const fortsym_expr *input[], size_t rank, const int variance[],
    int density_weight, size_t slot, fortsym_expr *out[], char *message,
    size_t capacity);
int fortsym_spacetime_tensor_lower(
    fortsym_arena *arena, const fortsym_expr *components[], int dimension,
    const fortsym_expr *coordinates[], const int signature[], int orientation,
    const fortsym_expr *input[], size_t rank, const int variance[],
    int density_weight, size_t slot, fortsym_expr *out[], char *message,
    size_t capacity);
int fortsym_spacetime_tensor_density_factor(
    fortsym_arena *arena, const fortsym_expr *components[], int dimension,
    const fortsym_expr *coordinates[], const int signature[], int orientation,
    const fortsym_expr *input[], size_t rank, const int variance[],
...
int fortsym_chart_tensor_permute(
    ...
    fortsym_expr *out[], char *message, size_t capacity);
/* Slots are one-based, matching the native Fortran API. */
int fortsym_chart_tensor_contract(
    fortsym_arena *arena, const fortsym_expr *coordinates[],
    const fortsym_expr *position[], const fortsym_expr *components[],
    size_t rank, const int variance[], int density_weight, size_t first_slot,
    size_t second_slot, fortsym_expr *out[], char *message, size_t capacity);
int fortsym_chart_tensor_product(
    fortsym_arena *arena, const fortsym_expr *coordinates[],
    const fortsym_expr *position[], const fortsym_expr *left_components[],
    size_t left_rank, const int left_variance[], int left_density_weight,
    const fortsym_expr *right_components[], size_t right_rank,
    const int right_variance[], int right_density_weight, fortsym_expr *out[],
    char *message, size_t capacity);
/* `antisymmetric` is zero for symmetrization and nonzero for
