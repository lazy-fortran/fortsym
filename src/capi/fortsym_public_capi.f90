module fortsym_public_capi
    ...
    use fortsym_spacetime_tensor, only: spacetime_tensor_t, &
        ...
        spacetime_tensor_symmetrize_native => spacetime_tensor_symmetrize, &
        spacetime_tensor_antisymmetrize_native => &
        spacetime_tensor_antisymmetrize, &
    ...
    use fortsym_matrix_adapter, only: calculate_matrix_det, calculate_matrix_trace, &
        ...
        calculate_matrix_is_identity, &
        calculate_matrix_is_echelon, &
        calculate_matrix_is_hermitian, &
    ...
    public :: fortsym_chart_sqrtg, fortsym_chart_surface_measure, &
        ...
        fortsym_chart_fourier_transverse_boundary_flux, &
        fortsym_chart_fourier_transverse_boundary_contraction, &
        ...
        fortsym_chart_connection_geodesic_residual, &
        ...
        fortsym_spacetime_tensor_symmetrize, &
...
contains
    ...
    function fortsym_chart_tensor_lie(raw, coordinates, position, &
            ...
            vector_density_weight, components, rank, variance, density_weight, &
            out, message, capacity) bind(c, name="fortsym_chart_tensor_lie") &
            result(status)
        type(c_ptr), value :: raw, coordinates, position, vector_components, &
            vector_variance, components, variance, out
        integer(c_size_t), value :: vector_rank, rank
        integer(c_int), value :: vector_density_weight, density_weight
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        type(arena_owner_t), pointer :: a
        type(chart_t) :: chart
        type(tensor_t) :: vector_value, input, value

        call get_chart_tensor_input(raw, coordinates, position, &
            vector_components, vector_rank, vector_variance, &
            vector_density_weight, chart, a, vector_value, status, message, &
            capacity)
        if (status /= FORTSYM_OK) return
        call get_chart_tensor_input(raw, coordinates, position, components, rank, &
            variance, density_weight, chart, a, input, status, message, capacity)
        if (status /= FORTSYM_OK) return
        if (vector_rank /= 1_c_size_t .or. vector_density_weight /= 0_c_int .or. &
            vector_value%variance(1) /= 1) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        value = tensor_lie_derivative(chart, vector_value, input)
        if (.not. tensor_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
    ...
    function fortsym_chart_connection_geodesic_residual(raw, coordinates, &
            position, components, curve, parameter, out, message, capacity) &
            bind(c, name="fortsym_chart_connection_geodesic_residual") &
            result(status)
        type(c_ptr), value :: raw, coordinates, position, components, curve, &
            parameter, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        ...
        value = geodesic_residual(connection, curve_value, parameter_value)
        call make_expr_array(a, value, out, DIM, status, message, capacity)
    end function fortsym_chart_connection_geodesic_residual
    ...
    function fortsym_chart_b_con_form(raw, coordinates, position, input, degree, &
            ...
            name="fortsym_chart_b_con_form") result(status)
        type(c_ptr), value :: raw, coordinates, position, input, out
        integer(c_size_t), value :: degree
        integer(c_int), value :: orientation
        character(kind=c_char), intent(out) :: message(*)
        ...
        call get_chart_inputs(raw, coordinates, position, int(DIM, c_size_t), &
            chart, a, status, message, capacity)
        if (status /= FORTSYM_OK) return
        if (degree /= 2_c_size_t .or. (orientation /= 1_c_int .and. &
            orientation /= -1_c_int)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        call get_form_input(chart, a, input, degree, input_value, status, &
            message, capacity)
        if (status /= FORTSYM_OK) return
        value = b_con_form(chart, input_value, int(orientation))
        call make_array_handles(a, value, output, status, message, capacity)
    end function fortsym_chart_b_con_form

    function fortsym_chart_b_density_form(raw, coordinates, position, input, degree, &
            orientation, out, message, capacity) bind(c, &
            name="fortsym_chart_b_density_form") result(status)
        type(c_ptr), value :: raw, coordinates, position, input, out
        integer(c_size_t), value :: degree
        integer(c_int), value :: orientation
        character(kind=c_char), intent(out) :: message(*)
        ...
        type(form_t) :: input_value
        type(tensor_t) :: value
        type(expr_t) :: components(DIM)
        type(c_ptr), pointer :: output(:)
        integer :: shape(1), indices(1), k
        ...
        call get_form_input(chart, a, input, degree, input_value, status, &
            message, capacity)
        if (status /= FORTSYM_OK) return
        value = b_density_form(chart, input_value, int(orientation))
        if (.not. tensor_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        do k = 1, DIM
            indices(1) = k
            components(k) = tensor_component(value, indices)
        end do
        call make_array_handles(a, components, output, status, message, capacity)
    end function fortsym_chart_b_density_form
    ...
    function fortsym_chart_fourier_weak_form(raw, coordinates, position, &
            ...
            transverse_curl_coefficient, transverse_mass, &
        ...
        type(c_ptr), value :: longitudinal_diffusion
        type(c_ptr), value :: transverse_curl_coefficient, transverse_mass
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        type(arena_owner_t), pointer :: a
        type(expr_owner_t), pointer :: owner
        type(chart_t) :: chart
        type(expr_t) :: nu(DIM, DIM), value_curl
        type(expr_t) :: value_diffusion(4), value_mass(4)
        ...
        call prepare_expr_array(longitudinal_diffusion, 4, status, message, &
            capacity)
        if (status /= FORTSYM_OK) return
        ...
        do j = 1, 2
            do i = 1, 2
                flat = flat + 1
                value_diffusion(flat) = form%longitudinal_diffusion(i, j)
        ...
        call make_handle(a, value_curl, transverse_curl_coefficient, status, &
            message, capacity)
        if (status /= FORTSYM_OK) return
        call make_expr_array(a, value_diffusion, longitudinal_diffusion, 4, &
            status, message, capacity)
        if (status /= FORTSYM_OK) return
    ...
    function fortsym_chart_fourier_longitudinal_boundary_flux(raw, coordinates, &
            position, reluctivity, potential, normal, out, message, capacity) &
            bind(c, name="fortsym_chart_fourier_longitudinal_boundary_flux") &
            result(status)
        type(c_ptr), value :: raw, coordinates, position, reluctivity, potential
        type(c_ptr), value :: normal, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        type(arena_owner_t), pointer :: a
        type(expr_owner_t), pointer :: owner
        type(chart_t) :: chart
        type(expr_t) :: nu(DIM, DIM), normal_value(2), potential_value, value
        type(fourier_constitutive_t) :: material
        type(c_ptr), pointer :: reluctivity_values(:), normal_values(:)
        integer :: i, j, flat, shape_matrix(1), shape_pair(1)
        ...
        shape_pair(1) = 2
        call c_f_pointer(reluctivity, reluctivity_values, shape_matrix)
        call c_f_pointer(normal, normal_values, shape_pair)
        call get_chart_inputs(raw, coordinates, position, int(DIM, c_size_t), &
            chart, a, status, message, capacity)
        if (status /= FORTSYM_OK) return
        call get_expr(potential, owner, potential_value, status, message, &
        ...
        do i = 1, 2
            call get_expr(normal_values(i), owner, normal_value(i), status, &
                message, capacity)
            if (status /= FORTSYM_OK) return
            if (.not. associated(owner%arena, a)) then
                call fail(status, message, capacity, FORTSYM_FOREIGN_ARENA)
        ...
        value = fourier_longitudinal_boundary_flux(chart, material, &
            potential_value, normal_value)
        if (.not. is_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_chart_fourier_longitudinal_boundary_flux

    function fortsym_chart_fourier_transverse_boundary_flux(raw, coordinates, &
            position, reluctivity, potential, normal, out, message, capacity) &
            bind(c, name="fortsym_chart_fourier_transverse_boundary_flux") &
            result(status)
        type(c_ptr), value :: raw, coordinates, position, reluctivity, potential
        type(c_ptr), value :: normal, out
        character(kind=c_char), intent(out) :: message(*)
        ...
        type(expr_owner_t), pointer :: owner
        type(chart_t) :: chart
        type(expr_t) :: nu(DIM, DIM), input(2), normal_value(2), value(2)
        type(fourier_constitutive_t) :: material
        type(c_ptr), pointer :: reluctivity_values(:), potential_values(:)
        type(c_ptr), pointer :: normal_values(:)
        integer :: i, j, flat, shape_matrix(1), shape_pair(1)
        ...
        call fourier_transverse_boundary_flux(chart, material, input, &
            normal_value, value)
        call make_expr_array(a, value, out, 2, status, message, capacity)
    end function fortsym_chart_fourier_transverse_boundary_flux

    function fortsym_chart_fourier_transverse_boundary_contraction(raw, &
            coordinates, position, reluctivity, potential, normal, test, out, &
            message, capacity) bind(c, &
            name="fortsym_chart_fourier_transverse_boundary_contraction") &
            result(status)
        type(c_ptr), value :: raw, coordinates, position, reluctivity, potential
        type(c_ptr), value :: normal, test, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        type(arena_owner_t), pointer :: a
        type(expr_owner_t), pointer :: owner
        type(chart_t) :: chart
        type(expr_t) :: nu(DIM, DIM), input(2), normal_value(2), test_value(2)
        type(expr_t) :: value
        type(fourier_constitutive_t) :: material
        type(c_ptr), pointer :: reluctivity_values(:), potential_values(:)
        type(c_ptr), pointer :: normal_values(:), test_values(:)
        integer :: i, j, flat, shape_matrix(1), shape_pair(1)
        ...
        call c_f_pointer(potential, potential_values, shape_pair)
        call c_f_pointer(normal, normal_values, shape_pair)
        call c_f_pointer(test, test_values, shape_pair)
        call get_chart_inputs(raw, coordinates, position, int(DIM, c_size_t), &
            chart, a, status, message, capacity)
        if (status /= FORTSYM_OK) return
        do i = 1, 2
            ...
            call get_expr(test_values(i), owner, test_value(i), status, &
                message, capacity)
            if (status /= FORTSYM_OK) return
            if (.not. associated(owner%arena, a)) then
                call fail(status, message, capacity, FORTSYM_FOREIGN_ARENA)
        ...
        value = fourier_transverse_boundary_contraction(chart, material, input, &
        ...
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_chart_fourier_transverse_boundary_contraction
    ...
    function fortsym_chart_fourier_longitudinal_residual(raw, coordinates, &
            ...
            bind(c, name="fortsym_chart_fourier_longitudinal_residual") &
            result(status)
        type(c_ptr), value :: raw, coordinates, position, reluctivity, potential
        type(c_ptr), value :: current, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        type(arena_owner_t), pointer :: a
        type(expr_owner_t), pointer :: owner
        type(chart_t) :: chart
        type(expr_t) :: nu(DIM, DIM), value
        type(expr_t) :: potential_value, current_value
        type(c_ptr), pointer :: reluctivity_values(:)
        integer :: i, j, flat, shape_matrix(1)
        ...
        call get_expr(current, owner, current_value, status, message, capacity)
        if (status /= FORTSYM_OK) return
        if (.not. associated(owner%arena, a)) then
            call fail(status, message, capacity, FORTSYM_FOREIGN_ARENA)
            return
        ...
        value = fourier_longitudinal_residual(chart, fourier_constitutive(chart, nu), &
        ...
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_chart_fourier_longitudinal_residual

    function fortsym_chart_fourier_transverse_residual(raw, coordinates, &
            position, reluctivity, potential, current, mode, out, message, &
            capacity) bind(c, &
            name="fortsym_chart_fourier_transverse_residual") result(status)
        type(c_ptr), value :: raw, coordinates, position, reluctivity, potential
        type(c_ptr), value :: current, mode, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        type(arena_owner_t), pointer :: a
        type(expr_owner_t), pointer :: owner
        type(chart_t) :: chart
        type(expr_t) :: nu(DIM, DIM), input(2), source(2), value(2)
        type(expr_t) :: mode_value
        type(fourier_constitutive_t) :: material
        type(c_ptr), pointer :: reluctivity_values(:), potential_values(:)
        type(c_ptr), pointer :: current_values(:), output(:)
        integer :: i, j, flat, shape_matrix(1), shape_pair(1)
        ...
        call c_f_pointer(reluctivity, reluctivity_values, shape_matrix)
        call c_f_pointer(potential, potential_values, shape_pair)
        call c_f_pointer(current, current_values, shape_pair)
        call c_f_pointer(out, output, shape_pair)
        call clear_array_outputs(output)
        call get_chart_inputs(raw, coordinates, position, int(DIM, c_size_t), &
            chart, a, status, message, capacity)
        if (status /= FORTSYM_OK) return
        do i = 1, 2
            ...
            call get_expr(current_values(i), owner, source(i), status, message, &
                capacity)
            if (status /= FORTSYM_OK) return
            if (.not. associated(owner%arena, a)) then
                call fail(status, message, capacity, FORTSYM_FOREIGN_ARENA)
        ...
        value = fourier_transverse_residual(chart, material, input, source, &
            mode_value)
        call make_expr_array(a, value, out, 2, status, message, capacity)
    end function fortsym_chart_fourier_transverse_residual
    ...
    function fortsym_spacetime_tensor_permute(raw, components, dimension, &
            coordinates, signature, orientation, input, rank, variance, &
            density_weight, order, out, message, capacity) bind(c, &
            name="fortsym_spacetime_tensor_permute") result(status)
        type(c_ptr), value :: raw, components, coordinates, signature, input
        type(c_ptr), value :: variance, order, out
        integer(c_int), value :: dimension, orientation, density_weight
        integer(c_size_t), value :: rank
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        type(spacetime_tensor_t) :: input_value, value
        integer(c_int), pointer :: order_values(:)
        integer :: order_fortran(SPACETIME_TENSOR_MAX_RANK)
        integer :: shape(1), k
        ...
        if (rank == 0_c_size_t) then
            call make_spacetime_tensor_array(a, input_value, 0, out, status, &
                message, capacity)
            return
        end if
        if (.not. c_associated(order)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        shape(1) = int(rank)
        ...
        value = spacetime_tensor_permute_native(input_value, &
            order_fortran(1:int(rank)))
        if (.not. spacetime_tensor_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        call make_spacetime_tensor_array(a, value, int(rank), out, status, &
            message, capacity)
    end function fortsym_spacetime_tensor_permute

    function fortsym_spacetime_tensor_contract(raw, components, dimension, &
            coordinates, signature, orientation, input, rank, variance, &
            density_weight, first_slot, second_slot, out, message, capacity) &
            bind(c, name="fortsym_spacetime_tensor_contract") result(status)
        type(c_ptr), value :: raw, components, coordinates, signature, input
        type(c_ptr), value :: variance, out
        integer(c_int), value :: dimension, orientation, density_weight
        integer(c_size_t), value :: rank, first_slot, second_slot
        ...
        value = spacetime_tensor_contract_native(input_value, int(first_slot), &
            int(second_slot))
        if (.not. spacetime_tensor_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
    ...
    function fortsym_spacetime_tensor_symmetrize(raw, components, dimension, &
            coordinates, signature, orientation, input, rank, variance, &
            density_weight, first_slot, second_slot, antisymmetric, out, &
            message, capacity) bind(c, &
            name="fortsym_spacetime_tensor_symmetrize") result(status)
        type(c_ptr), value :: raw, components, coordinates, signature, input, out
        type(c_ptr), value :: variance
        integer(c_int), value :: dimension, orientation, density_weight
        integer(c_int), value :: antisymmetric
        integer(c_size_t), value :: rank, first_slot, second_slot
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        ...
        if (antisymmetric == 0_c_int) then
            value = spacetime_tensor_symmetrize_native(input_value, &
                int(first_slot), int(second_slot))
        else
            value = spacetime_tensor_antisymmetrize_native(input_value, &
                int(first_slot), int(second_slot))
        end if
        if (.not. spacetime_tensor_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
        ...
        call make_spacetime_tensor_array(a, value, int(rank), out, status, &
            message, capacity)
    end function fortsym_spacetime_tensor_symmetrize
    ...
    function fortsym_spacetime_tensor_product(raw, components, dimension, &
            ...
            left_density_weight, right, right_rank, right_variance, &
            right_density_weight, out, message, capacity) bind(c, &
            name="fortsym_spacetime_tensor_product") result(status)
        type(c_ptr), value :: raw, components, coordinates, signature, left, right
        type(c_ptr), value :: left_variance, right_variance, out
        integer(c_int), value :: dimension, orientation
        integer(c_int), value :: left_density_weight, right_density_weight
        integer(c_size_t), value :: left_rank, right_rank
        character(kind=c_char), intent(out) :: message(*)
        ...
        type(arena_owner_t), pointer :: a
        type(spacetime_metric_t) :: metric
        type(spacetime_tensor_t) :: left_value, right_value, value
        integer :: output_rank

        call get_spacetime_metric_input(raw, components, dimension, coordinates, &
            signature, orientation, a, metric, status, message, capacity)
        if (status /= FORTSYM_OK) return
        call get_spacetime_tensor_input(metric, a, left, left_rank, left_variance, &
            left_density_weight, left_value, status, message, capacity)
        if (status /= FORTSYM_OK) return
        call get_spacetime_tensor_input(metric, a, right, right_rank, &
            right_variance, right_density_weight, right_value, status, message, &
            capacity)
        if (status /= FORTSYM_OK) return
        if (left_rank + right_rank > int(SPACETIME_TENSOR_MAX_RANK, c_size_t)) then
            call fail(status, message, capacity, FORTSYM_RESOURCE_LIMIT)
            return
        end if
        value = spacetime_tensor_product_native(left_value, right_value)
        if (.not. spacetime_tensor_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        output_rank = int(left_rank + right_rank)
        call make_spacetime_tensor_array(a, value, output_rank, out, status, &
            message, capacity)
    end function fortsym_spacetime_tensor_product

    function fortsym_spacetime_tensor_covariant_diff(raw, components, dimension, &
            coordinates, signature, orientation, input, rank, variance, &
            density_weight, out, message, capacity) bind(c, &
            name="fortsym_spacetime_tensor_covariant_diff") result(status)
        type(c_ptr), value :: raw, components, coordinates, signature, input, out
        type(c_ptr), value :: variance
        integer(c_int), value :: dimension, orientation, density_weight
        integer(c_size_t), value :: rank
        ...
        value = spacetime_tensor_covariant_diff_native(metric, input_value)
        if (.not. spacetime_tensor_valid(value)) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
    ...
    function fortsym_spacetime_form_volume(raw, components, dimension, coordinates, &
            signature, orientation, out, message, capacity) bind(c, &
            name="fortsym_spacetime_form_volume") result(status)
        type(c_ptr), value :: raw, components, coordinates, signature, out
        integer(c_int), value :: dimension, orientation
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
    ...
    function fortsym_linsolve_parametric( &
            raw, matrix_raw, right_hand_side_raw, variables_raw, &
            equation_count, variable_count, out, output_capacity, count, &
            message, capacity) bind(c, name="fortsym_linsolve_parametric") &
            result(status)
        type(c_ptr), value :: raw, matrix_raw, right_hand_side_raw
        type(c_ptr), value :: variables_raw, out
        integer(c_size_t), value :: equation_count, variable_count
        ...
        type(expr_owner_t), pointer :: owner
        type(c_ptr), pointer :: raw_matrix(:), raw_right_hand_side(:)
        type(c_ptr), pointer :: raw_variables(:), output(:)
        type(expr_t), allocatable :: matrix(:, :), right_hand_side(:)
        type(expr_t), allocatable :: variables(:), values(:)
        integer :: column, index, equations, shape(1), variable, variables_count
        logical :: consistent, ok
        character(:), allocatable :: why
        ...
        if (.not. c_associated(matrix_raw) .or. &
            .not. c_associated(right_hand_side_raw) .or. &
            .not. c_associated(variables_raw) .or. .not. c_associated(out) .or. &
            equation_count < 1_c_size_t .or. &
            variable_count < 1_c_size_t .or. &
            equation_count > int(MAX_LINSOLVE_DIM, c_size_t) .or. &
            variable_count > int(MAX_LINSOLVE_DIM - 1, c_size_t) .or. &
            output_capacity < variable_count) then
            call fail(status, message, capacity, FORTSYM_INVALID_ARGUMENT)
            return
        end if
        if (output_capacity > int(huge(0), c_size_t)) then
        ...
        variables_count = int(variable_count)
        shape(1) = int(equation_count*variable_count)
        call c_f_pointer(matrix_raw, raw_matrix, shape)
        shape(1) = equations
        call c_f_pointer(right_hand_side_raw, raw_right_hand_side, shape)
        shape(1) = variables_count
        call c_f_pointer(variables_raw, raw_variables, shape)
        shape(1) = int(output_capacity)
        call c_f_pointer(out, output, shape)
        call clear_handles(output, shape(1))
        ...
        call get_arena(raw, a, status, message, capacity)
        if (status /= FORTSYM_OK) return
        allocate (matrix(equations, variables_count))
        allocate (right_hand_side(equations))
        allocate (variables(variables_count))
        do column = 1, variables_count
            do variable = 1, equations
                index = (column - 1)*equations + variable
                call get_expr(raw_matrix(index), owner, matrix(variable, column), &
                    status, message, capacity)
                if (status /= FORTSYM_OK) return
                if (.not. associated(owner%arena, a)) then
                    call fail(status, message, capacity, FORTSYM_FOREIGN_ARENA)
        ...
        do variable = 1, equations
            call get_expr( &
                raw_right_hand_side(variable), owner, right_hand_side(variable), &
                status, message, capacity)
            if (status /= FORTSYM_OK) return
            if (.not. associated(owner%arena, a)) then
                call fail(status, message, capacity, FORTSYM_FOREIGN_ARENA)
        ...
        do variable = 1, variables_count
            call get_expr(raw_variables(variable), owner, variables(variable), &
                status, message, capacity)
            if (status /= FORTSYM_OK) return
            if (.not. associated(owner%arena, a)) then
                call fail(status, message, capacity, FORTSYM_FOREIGN_ARENA)
        ...
        call calculate_parametric_linsolve( &
            a%engine, matrix, right_hand_side, variables, values, consistent, &
            ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        ...
        call make_expr_array( &
            a, values, out, variables_count, status, message, capacity)
    end function fortsym_linsolve_parametric

    function fortsym_matrix_det(raw, expression_raw, out, message, capacity) &
            bind(c, name="fortsym_matrix_det") result(status)
        type(c_ptr), value :: raw, expression_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
    ...
    function fortsym_matrix_trace(raw, expression_raw, out, message, capacity) &
        ...
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_matrix_trace

    function fortsym_matrix_is_diagonal(raw, expression_raw, verdict, message, capacity) &
            bind(c, name="fortsym_matrix_is_diagonal") result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_diagonal( &
    ...
    function fortsym_matrix_is_zero_matrix(raw, expression_raw, verdict, &
            message, capacity) bind(c, name="fortsym_matrix_is_zero_matrix") &
            result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_zero_matrix( &
            a%value, a%engine, expression, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        ...
        status = FORTSYM_OK
    end function fortsym_matrix_is_zero_matrix

    function fortsym_matrix_is_upper(raw, expression_raw, verdict, message, capacity) &
            bind(c, name="fortsym_matrix_is_upper") result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_upper( &
            a%value, a%engine, expression, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        ...
        status = FORTSYM_OK
    end function fortsym_matrix_is_upper

    function fortsym_matrix_is_lower(raw, expression_raw, verdict, message, capacity) &
            bind(c, name="fortsym_matrix_is_lower") result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_lower( &
            a%value, a%engine, expression, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
    ...
    function fortsym_matrix_is_identity(raw, expression_raw, verdict, message, capacity) &
            bind(c, name="fortsym_matrix_is_identity") result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_identity( &
            a%value, a%engine, expression, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        ...
        status = FORTSYM_OK
    end function fortsym_matrix_is_identity

    function fortsym_matrix_is_echelon(raw, expression_raw, verdict, message, capacity) &
            bind(c, name="fortsym_matrix_is_echelon") result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_echelon( &
            a%value, a%engine, expression, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        ...
        status = FORTSYM_OK
    end function fortsym_matrix_is_echelon

    function fortsym_matrix_is_hermitian(raw, expression_raw, verdict, message, capacity) &
            bind(c, name="fortsym_matrix_is_hermitian") result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_hermitian( &
            a%value, a%engine, expression, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        ...
        status = FORTSYM_OK
    end function fortsym_matrix_is_hermitian
    ...
    function fortsym_matrix_is_anti_symmetric(raw, expression_raw, simplify, verdict, &
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_anti_symmetric( &
            a%value, a%engine, expression, simplify /= 0, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        end if
        status = FORTSYM_OK
    end function fortsym_matrix_is_anti_symmetric

    function fortsym_matrix_is_symbolic(raw, expression_raw, verdict, message, capacity) &
            bind(c, name="fortsym_matrix_is_symbolic") result(status)
        type(c_ptr), value :: raw, expression_raw
        integer(c_int), intent(out) :: verdict
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        ...
        call prepare_native_engine(a)
        call calculate_matrix_is_symbolic( &
            a%value, a%engine, expression, verdict, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        ...
        status = FORTSYM_OK
    end function fortsym_matrix_is_symbolic
    ...
    function fortsym_matrix_rank(raw, expression_raw, out, message, capacity) &
        ...
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_matrix_rank

    function fortsym_matrix_inverse(raw, expression_raw, out, message, capacity) &
            bind(c, name="fortsym_matrix_inverse") result(status)
        type(c_ptr), value :: raw, expression_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        ...
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_matrix_inverse

    function fortsym_matrix_transpose(raw, expression_raw, out, message, capacity) &
            bind(c, name="fortsym_matrix_transpose") result(status)
        type(c_ptr), value :: raw, expression_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        ...
        call calculate_matrix_transpose( &
            a%value, expression, value, ok, why)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        end if
    ...
    function fortsym_matrix_multiply_elementwise( &
            raw, left_raw, right_raw, out, message, capacity) bind(c, &
            name="fortsym_matrix_multiply_elementwise") result(status)
        type(c_ptr), value :: raw, left_raw, right_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        ...
        call calculate_matrix_multiply_elementwise( &
        ...
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_matrix_multiply_elementwise
    ...
    subroutine matrix_binary_operation(raw, left_raw, right_raw, out, message, &
        ...
        call calculate_matrix_add( &
            a%value, a%engine, left, right, value, ok, why, subtract)
        if (.not. ok) then
            call fail_reason(status, message, capacity, FORTSYM_UNSUPPORTED, why)
            return
        end if
        call make_handle(a, value, out, status, message, capacity)
    end subroutine matrix_binary_operation

    function fortsym_matrix_add(raw, left_raw, right_raw, out, message, capacity) &
            bind(c, name="fortsym_matrix_add") result(status)
        type(c_ptr), value :: raw, left_raw, right_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status

        call matrix_binary_operation( &
            raw, left_raw, right_raw, out, message, capacity, .false., status)
    end function fortsym_matrix_add

    function fortsym_matrix_subtract(raw, left_raw, right_raw, out, message, capacity) &
            bind(c, name="fortsym_matrix_subtract") result(status)
        type(c_ptr), value :: raw, left_raw, right_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
        ...
        call matrix_binary_operation( &
            raw, left_raw, right_raw, out, message, capacity, .true., status)
    end function fortsym_matrix_subtract

    function fortsym_matrix_negate(raw, expression_raw, out, message, capacity) &
            bind(c, name="fortsym_matrix_negate") result(status)
        type(c_ptr), value :: raw, expression_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
    ...
    function fortsym_matrix_nullspace(raw, expression_raw, out, message, capacity) &
        ...
        call make_handle(a, value, out, status, message, capacity)
    end function fortsym_matrix_nullspace

    function fortsym_matrix_rref(raw, expression_raw, out, message, capacity) &
            bind(c, name="fortsym_matrix_rref") result(status)
        type(c_ptr), value :: raw, expression_raw, out
        character(kind=c_char), intent(out) :: message(*)
        integer(c_size_t), value :: capacity
        integer(c_int) :: status
