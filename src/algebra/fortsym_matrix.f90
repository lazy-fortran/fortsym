...
contains
    ...
    !> Boolean triangular predicates using the caller's native zero oracle.
    subroutine matrix_is_upper(a, engine, e, verdict, ok, why)
        type(arena_t), target, intent(inout) :: a
        class(engine_t), intent(inout) :: engine
        type(expr_t), intent(in) :: e
        integer, intent(out) :: verdict
        ...
        call matrix_is_triangular( &
            engine, e, .true., verdict, ok, why)
    end subroutine matrix_is_upper

    subroutine matrix_is_lower(a, engine, e, verdict, ok, why)
        type(arena_t), target, intent(inout) :: a
        class(engine_t), intent(inout) :: engine
        type(expr_t), intent(in) :: e
        integer, intent(out) :: verdict
        ...
        call matrix_is_triangular( &
            engine, e, .false., verdict, ok, why)
    end subroutine matrix_is_lower

    subroutine matrix_is_triangular(engine, e, upper, verdict, ok, why)
        class(engine_t), intent(inout) :: engine
        type(expr_t), intent(in) :: e
        ...
        if (rows == 0) then
            if (upper) then
                why = str("Upper-triangular test on something that is not a matrix")
            else
                why = str("Lower-triangular test on something that is not a matrix")
            end if
            return
        end if
