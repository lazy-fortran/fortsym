program gen_matern12_hvp
    ! Generate the complete value/JVP/VJP/HVP leaf for the Matérn-1/2
    ! covariance.  The source is intentionally kept in FortSym: this is the
    ! proof-producing counterpart of FortAD's generic HVP generator and lets
    ! consumers compare a single hash-consed DAG with the checked-in AD leaf.
    ...
    if (output_length == 0 .or. revision_length == 0) then
        write (output_unit, '(a)') &
            'usage: gen_matern12_hvp OUTPUT_PATH FORTSYM_REVISION'
        error stop 2
    end if
    ...
    value = exp_expr(lv - distance*exp_expr(-ll))
    ...
    spec%name = str('fortml_generated_matern12_hvp_core')
    ...
    spec%generator = str('gen_matern12_hvp')
    spec%generator_revision = str(trim(revision(:revision_length)))
    spec%regenerate_command = str( &
        'fo exec gen_matern12_hvp OUTPUT_PATH FORTSYM_REVISION')
    ...
    write (unit, '(a)') 'module fortml_generated_matern12_products'
    ...
    write (unit, '(a)') '        call fortml_generated_matern12_hvp_core(distance, distance_d, lv, lv_d, ll, ll_d, y_b, &'
    write (unit, '(a)') '            y, y_d, distance_b, distance_b_d, lv_b, lv_b_d, ll_b, ll_b_d)'
    write (unit, '(a)') '    end subroutine fortml_matern12_hvp'
    write (unit, '(a)') 'end module fortml_generated_matern12_products'
    close (unit)
