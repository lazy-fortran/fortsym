program gen_matern52_hvp
    ...
    if (output_length == 0 .or. revision_length == 0) error stop 2
    ...
    value = exp_expr(lv)*(1.0_dp + sqrt(5.0_dp)*distance*exp_expr(-ll) + &
        5.0_dp*distance*distance*exp_expr(-2.0_dp*ll)/3.0_dp)* &
        exp_expr(-sqrt(5.0_dp)*distance*exp_expr(-ll))
    ...
    do k = 1, size(roots)
        ...
        if (.not. simplified%ok) error stop 1
        roots(k) = simplified%value
    end do
    call lower_kernel_ir(roots, ir, good, message)
    if (.not. good) error stop 1
    spec%name = str('fortml_generated_matern52_hvp_core')
    spec%args = [str('distance'), str('distance_d'), str('lv'), str('lv_d'), &
        str('ll'), str('ll_d'), str('y_b')]
    spec%outputs = [str('y'), str('y_d'), str('distance_b'), str('distance_b_d'), &
        str('lv_b'), str('lv_b_d'), str('ll_b'), str('ll_b_d')]
    spec%temp_prefix = str('fs_t')
    spec%generator = str('gen_matern52_hvp')
    spec%generator_revision = str(trim(revision(:revision_length)))
    spec%regenerate_command = str('fo exec gen_matern52_hvp OUTPUT_PATH FORTSYM_REVISION')
    ...
    write (unit, '(a)') 'module fortml_generated_matern52_products'
    write (unit, '(a)') '    implicit none'
    write (unit, '(a)') 'contains'
    write (unit, '(a)') chars(source)
    write (unit, '(a)') '    subroutine fortml_matern52_hvp(distance, distance_d, lv, lv_d, ll, ll_d, y, y_d, &'
    write (unit, '(a)') '            y_b, distance_b, distance_b_d, lv_b, lv_b_d, ll_b, ll_b_d)'
    ...
    write (unit, '(a)') '        call fortml_generated_matern52_hvp_core(distance, distance_d, lv, lv_d, ll, ll_d, y_b, &'
    write (unit, '(a)') '            y, y_d, distance_b, distance_b_d, lv_b, lv_b_d, ll_b, ll_b_d)'
    write (unit, '(a)') '    end subroutine fortml_matern52_hvp'
    write (unit, '(a)') 'end module fortml_generated_matern52_products'
    close (unit)
