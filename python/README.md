...
vector. A native augmented `Matrix` may also be passed directly, and symbols
may be given as one tuple/list or as separate positional arguments. Symbolic
coefficients and other alternate forms remain explicit refusals.
...
`is_upper_hessenberg`, `is_lower_hessenberg`, and
`is_Identity`, and
`is_echelon`, and
`is_hermitian`, and
...
`rref()` returns `(reduced_matrix, pivot_tuple)`. The bounded exact
`charpoly(x)` and `eigenvals(multiple=False)` methods cover square matrices
whose verified characteristic roots are in the native solver fragment;
`eigenvals(multiple=True)` preserves repeated roots. Ragged, singular matrices,
unsupported characteristic-root degree, and broader options remain explicit
refusals.
...
| `integrate` | verified one-variable indefinite integration in the SymPy-compatible default complex domain, plus finite definite `(variable, lower, upper)` integration when continuity and the fundamental-theorem hypotheses are proved; unsupported antiderivatives, improper intervals, multiple variables, and options are explicit refusals |
| `summation`, `product` | bounded finite integer ranges use the native direct-expansion, closed-form, and telescoping owner; unsupported convergence, infinite bounds, hypergeometric forms, and options are explicit refusals |
...
| `solve_univariate_inequality`, `reduce_inequalities` | bounded exact affine one-variable inequalities, including strict/non-strict bounds and slope reversal, through one native owner; higher-degree, compound-domain, and `relational=False` forms are explicit refusals |
