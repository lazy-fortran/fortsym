...
`.T`, `nullspace()`, and `rref()`, delegating exact matrix operations to the
...
| `Matrix` | bounded exact dense construction, `(row, column)` indexing, native-backed determinant, exact rank/inverse, transpose/`.T`, bounded `nullspace()`, and `rref()`; ragged, singular, and broader matrix operations are explicit refusals |
