...
| `subs`, `subs_many`, `expand` | native substitution and expansion; `subs_many` is the paired-sequence native spelling, while `fortsym.sympy.subs(..., simultaneous=True)` matches SymPy's non-cascading replacement semantics |
