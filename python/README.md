...
`.T`, `nullspace()`, `rref()`, and multiplication with `*`/`@`, delegating exact matrix operations to the
...
| `Matrix` | bounded exact dense construction, `(row, column)` indexing, native-backed determinant, exact rank/inverse, transpose/`.T`, bounded `nullspace()`, `rref()`, and exact `*`/`@` products or scalar scaling; ragged, singular, and broader matrix operations are explicit refusals |
