...
column)` indexing, `det()`/`det(matrix)`, `rank()`, `inv()`, `transpose()`, and
`.T`, delegating exact matrix operations to the native owner. Ragged, singular
matrices and broader options remain explicit
refusals.
...
| `Matrix` | bounded exact dense construction, `(row, column)` indexing, native-backed determinant, exact rank/inverse, transpose/`.T`, and bounded `nullspace()`; ragged, singular, and broader matrix operations are explicit refusals |
