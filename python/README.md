...
| `Matrix` | bounded exact dense construction including flat sequences as column matrices, row-major flat scalar indexing and slicing, `(row, column)` indexing, `len()` and `is_square` shape metadata, row/column/block/reverse/stepped/empty 2-D slices, native-backed determinant and trace, exact rank including `simplify=True`, inverse, transpose/`.T`, bounded `nullspace()` with `simplify=True`, `rref()` with SymPy-shaped `pivots=False` and `simplify=True` options, elementwise `+`/`-`, unary negation, and exact `*`/`@` products or scalar scaling/division; ragged, non-square trace inputs, singular, callback-controlled zero tests, and broader matrix-expression operations are explicit refusals |
