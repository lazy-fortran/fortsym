...
| `subs`, `subs_many`, `expand` | native substitution and expansion; unordered mappings follow SymPy's node-count/structural ordering, explicit sequences retain caller order, `subs_many` is the paired-sequence native spelling, and `fortsym.sympy.subs(..., simultaneous=True)` matches SymPy's non-cascading replacement semantics |
