...
column)` indexing, `det()`/`det(matrix)`, and `rank()`, delegating exact matrix
operations to the native owner. Ragged matrices and broader matrix options remain explicit
refusals.
...
| `Matrix` | bounded exact dense construction, `(row, column)` indexing, native-backed determinant, and exact rank; ragged rows and broader matrix operations are explicit refusals |
