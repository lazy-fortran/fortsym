...
| `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `sinh`, `cosh`, `tanh`, `asinh`, `acosh`, `atanh`, `exp`, `log`, `sqrt`, `Abs`, `sign`, `floor`, `ceiling` | native applied-function nodes; direct sentinel rules for the declared inverse and direct heads match SymPy where the result is representable, while accumulation-bound `atan(zoo)` and `atanh(zoo)` remain explicit refusals |
