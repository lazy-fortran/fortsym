...
points, and exact `acosh(0)`/`acosh(-1)`, gamma-family domain-head
...
0.029x and 0.062x. The final matrix has 72 rows, 66 enforced rows, and zero
...
recorded ratios were 0.018x and 0.051x. The final matrix has 74 rows, 68
...
74-workload parity run also passed with zero correctness failures and zero
