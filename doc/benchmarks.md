...
points, exact `atanh(1)`/`atanh(-1)`, exact `atanh(±i)` branch points, and exact
`acosh(0)`/`acosh(-1)`, gamma-family domain-head
...
unwaived violations before the `acosh` rows are added. The
...
0.017x and 0.058x. The final matrix has 68 rows, 62 enforced rows, and zero
...
0.029x and 0.062x. The final matrix has 70 rows, 64 enforced rows, and zero
...
recorded ratios were 0.018x and 0.051x. The final matrix has 72 rows, 66
...
72-workload parity run also passed with zero correctness failures and zero
...
0.044× warm; `domain_acosh_branch` was 0.017× cold and 0.058× warm;
