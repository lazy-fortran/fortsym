...
density views, `fortsym_tensor` owns typed components, `fortsym_form` owns
exterior algebra, and `fortsym_connection` owns covariant/curvature views.
The Python `Chart`, `Tensor`, and `Form` classes only transport handles and
metadata through the C ABI.
