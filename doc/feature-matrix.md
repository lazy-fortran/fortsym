...
| Linear and polynomial solve | partial: distinct verified roots for one equation in one symbol and a bounded `solveset` `FiniteSet`/`EmptySet` wrapper; exact univariate polynomial roots and verified scalar-linear fallback are public through C/Python/SymPy facades | no public operation | Yacas concrete method | domains/conditions, systems, inequalities, and broader polynomial roots |
