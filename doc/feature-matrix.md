...
| Connections and curvature | native fixed-3D subset: typed Christoffel, `covariant_diff`, first-slot `covariant_divergence`, Riemann, Ricci, scalar curvature, and Einstein tensor from chart or coordinate-aware `metric_t`, plus metric compatibility and density-weight checks; C ABI and Python/SymPy chart views | no public nodes | no | pseudo-Riemannian sign-convention corpus, torsion/nonmetricity, geodesics, Bianchi identities, and full differential comparison |
