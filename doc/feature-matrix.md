...
| Coordinate charts, reciprocal bases, metrics, `sqrtg`, magnetic components, and Fourier modes | native first subset: charts, signed Jacobian, covariant and reciprocal basis matrices, metric, `grad`, `div`, `curl`, `laplacian`, `B^i`, `B_i`, `sqrtg B^i`, symmetry-mode curl, and bidirectional chart-map tensor transport; Python `Chart`/`ChartMap` transport facade | conversion/oracle path | partial | explicit dimensions, chart-map validation and composition, flux-coordinate descriptors, and paper weak forms |
