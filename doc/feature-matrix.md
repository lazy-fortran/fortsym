...
| Connections and curvature | native fixed-3D subset: typed Christoffel and `covariant_diff` from chart or coordinate-aware `metric_t`, Riemann, Ricci, scalar curvature, Einstein tensor, metric compatibility, and density-weight checks; C ABI and Python/SymPy chart views | no public nodes | no | full metric-owner curvature calculus, pseudo-Riemannian signatures, torsion/nonmetricity, geodesics, Bianchi identities, and full differential comparison |
