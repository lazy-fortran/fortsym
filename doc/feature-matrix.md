...
| Linear and polynomial solve | partial: distinct verified roots for one equation in one symbol, a bounded `solveset` `FiniteSet`/`EmptySet`/`Complement` wrapper with native symbolic rational-pole transport, and verified square exact-rational one-RHS `linsolve` returning `FiniteSet(Tuple(...))`; `Tuple` is a native-owned immutable application with SymPy-shaped indexing and printing; exact univariate polynomial roots, bounded rational-function numerator and denominator roots, scalar-linear fallback, and the bounded system path are public through C/Python/SymPy facades | no public operation | Yacas concrete method | domains/conditions beyond rational denominator poles, free parameters, symbolic systems, inequalities, and broader polynomial roots |
