...
| Symbolic matrices and general symbolic tensors | verified dense rational systems over `expr_t` arrays with one-RHS `linsolve` transport; coordinate tensors have a separate typed owner | no public matrix nodes | no | symbolic matrix construction, conditional pivots, free parameters, and broader matrix operations |
