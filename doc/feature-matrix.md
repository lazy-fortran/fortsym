...
| Connections and curvature | native fixed-3D subset: typed Christoffel from chart or coordinate-aware `metric_t`, covariant derivative, Riemann, Ricci, scalar curvature, Einstein tensor, metric compatibility, and density-weight checks; C ABI and Python/SymPy chart views | no public nodes | no | full metric-owner covariant calculus, pseudo-Riemannian signatures, torsion/nonmetricity, geodesics, Bianchi identities, and full differential comparison |
