...
| Coordinate charts, reciprocal bases, metrics, `sqrtg`, magnetic components, and Fourier modes | native first subset: charts, signed Jacobian, covariant and reciprocal basis matrices, metric, `grad`, ordinary `div`/`curl`, explicit `div_density`/`curl_density`, `laplacian`, `B^i`, `B_i`, `sqrtg B^i`, symmetry-mode curl, generic Fourier curl-curl/current, bidirectional chart-map tensor/form transport, fixed-3D map composition, and identically-singular-Jacobian refusal; Python `Chart`/`ChartMap` transport facade | conversion/oracle path | partial | explicit dimensions, separate positive density versus orientation, pseudo-Riemannian signatures, flux-coordinate descriptors, and paper weak forms |
...
| Differential forms and de Rham identities | native fixed-3D coordinate forms: `form_t`, `d`, wedge, metric `star`, interior product, Cartan `lie`, `flat`, `sharp`, magnetic `i_B(volume)=dA`, degree-four zero extension, and chart-map pullback checks | C ABI plus `fortsym.Form`/`fortsym.sympy.Form` transport and the first native-backed `Manifold`/`Patch`/`CoordSystem`/`Differential`/`WedgeProduct`/`LieDerivative` names | no | arbitrary dimension/rank, coordinate transformations beyond fixed-3D maps, Hodge signatures, Maxwell vocabulary, patch/topology metadata, and full Python/SymPy `diffgeom` parity |
