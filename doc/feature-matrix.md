...
| Linear and polynomial solve | partial: distinct verified roots for one equation in one symbol, a bounded `solveset` `FiniteSet`/`EmptySet` wrapper, and verified square exact-rational one-RHS `linsolve` returning `FiniteSet(Tuple(...))`; exact univariate polynomial roots, bounded rational-function numerator roots verified against the original residual, scalar-linear fallback, and the bounded system path are public through C/Python/SymPy facades | no public operation | Yacas concrete method | domains/conditions, free parameters, symbolic systems, inequalities, and broader polynomial roots |
