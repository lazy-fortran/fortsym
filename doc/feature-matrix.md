...
| Simplification | partial: principal-square-root powers, guarded `sqrt`/`abs`, exact `log(0)=zoo` with sentinel propagation, and real/nonzero-guarded `log`/`exp` compositions | partial | partial | broader guarded elementary rewrites and polynomial normal form |
