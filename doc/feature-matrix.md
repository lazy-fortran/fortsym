...
| Linear and polynomial solve | partial: distinct verified roots for one equation in one symbol, a bounded native-owned `solveset` `FiniteSet` plus singleton `EmptySet` and native-owned `Complement` result with native symbolic rational-pole transport, and verified square exact-rational one-RHS `linsolve` returning `FiniteSet(Tuple(...))`; `FiniteSet`, `Tuple`, and `Complement` are native-owned immutable applications with SymPy-shaped membership, indexing, and printing; exact univariate polynomial roots, bounded rational-function numerator and denominator roots, scalar-linear fallback, and the bounded system path are public through C/Python/SymPy facades | no public operation | Yacas concrete method | domains/conditions beyond rational denominator poles, free parameters, symbolic systems, inequalities, and broader polynomial roots |
...
| Symbolic matrices and general symbolic tensors | bounded exact dense `Matrix` construction, row-major flat scalar indexing and slicing, 2-D indexing, row/column/block/reverse/stepped/empty 2-D slicing, determinant, rank, inverse, transpose, `nullspace()`, `rref()`, products/scaling, scalar division, elementwise add/subtract, and negation through the native nested-`List` owner, plus verified rational systems over `expr_t` arrays with one-RHS `linsolve` transport; coordinate tensors have a separate typed owner | no public matrix-expression nodes | no | conditional pivots, free parameters, matrix expressions, and broader matrix operations |
