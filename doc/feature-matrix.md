...
| Coordinate charts, reciprocal bases, metrics, `sqrtg`, magnetic components, and Fourier modes | native first subset: charts, signed Jacobian, covariant and reciprocal basis matrices, metric, `grad`, `div`, `curl`, `laplacian`, `B^i`, `B_i`, `sqrtg B^i`, and symmetry-mode curl; Python `Chart` transport facade | conversion/oracle path | partial | explicit dimensions, coordinate-change composition, flux-coordinate descriptors, and paper weak forms |
