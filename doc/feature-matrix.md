...
| Differential forms and de Rham identities | native fixed-3D coordinate forms and dimension-aware spacetime forms: `form_t`/`spacetime_form_t`, `d`, chart/metric-owner oriented volume forms, metric `star`, `codifferential`/`codiff`, Laplace--de Rham, interior product, Cartan `lie`, `flat`, `sharp`, magnetic-chart potential/flux forms with `i_B(volume)=dA`, degree-four zero extension, chart-map pullback checks, and the first Maxwell owner for `F=d(A)`, gauge shifts, and `d(*F)-J` | C ABI plus `fortsym.Form`/`fortsym.SpacetimeForm`/`fortsym.sympy` transport and the first native-backed `Manifold`/`Patch`/`CoordSystem`/`Differential`/`WedgeProduct`/`LieDerivative` names | no | arbitrary dimension/rank, coordinate transformations beyond fixed-3D maps, full Maxwell media/topology, patch/boundary metadata, and full Python/SymPy `diffgeom` parity |
