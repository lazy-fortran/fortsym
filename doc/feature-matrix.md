...
| Symbolic matrices and general symbolic tensors | bounded exact dense `Matrix` construction including flat-sequence column matrices, shape metadata (`len()` and `is_square`), row-major flat scalar indexing and slicing, 2-D indexing, row/column/block/reverse/stepped/empty 2-D slices, determinant, trace, tri-state `is_diagonal()` and `is_zero_matrix`, boolean `is_upper`, `is_lower`, `is_anti_symmetric(simplify=True/False)`, `is_symbolic()`, `is_upper_hessenberg`, `is_lower_hessenberg`, `is_Identity`, `is_echelon`, `is_hermitian`, and `is_symmetric(simplify=True/False)`, elementwise conjugation, adjoint, and `H`, rank including `simplify=True`, inverse, transpose, `nullspace()` including `simplify=True`, `rref()` including SymPy-shaped `pivots=False` and `simplify=True`, products/scaling, scalar division, elementwise add/subtract, and negation through the native nested-`List` owner, plus verified rational systems over `expr_t` arrays with rectangular one-RHS `linsolve` transport; coordinate tensors have a separate typed owner | no public matrix-expression nodes | no | conditional pivots, callback-controlled zero tests, symbolic coefficients, unknown complex conjugates, non-square trace, undecidable predicates outside the native zero fragment, matrix expressions, and broader matrix operations |
