...
| Linear and polynomial solve | partial: distinct verified roots for one equation in one symbol, a bounded native-owned `solveset` `FiniteSet` plus singleton `EmptySet` and `Complement` result with native symbolic rational-pole transport, and verified square exact-rational one-RHS `linsolve` returning `FiniteSet(Tuple(...))`; `FiniteSet` and `Tuple` are native-owned immutable applications with SymPy-shaped membership, indexing, and printing; exact univariate polynomial roots, bounded rational-function numerator and denominator roots, scalar-linear fallback, and the bounded system path are public through C/Python/SymPy facades | no public operation | Yacas concrete method | domains/conditions beyond rational denominator poles, free parameters, symbolic systems, inequalities, and broader polynomial roots |
