...
| Symbolic matrices and general symbolic tensors | bounded exact dense `Matrix` construction, indexing, determinant, rank, inverse, transpose, `nullspace()`, `rref()`, products/scaling, scalar division, elementwise add/subtract, and negation through the native nested-`List` owner, plus verified rational systems over `expr_t` arrays with one-RHS `linsolve` transport; coordinate tensors have a separate typed owner | no public matrix-expression nodes | no | matrix slicing, conditional pivots, free parameters, and broader matrix operations |
