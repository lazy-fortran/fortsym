...
- The ownership-safe bounded `qqbar` bridge now provides a lossless
  ...
  projection. Native simplification also collects algebraic coefficients in
  mixed expressions and reduces their integer powers through FLINT. Higher-
  degree conversion and complex code generation remain next. Use Calcium or
  Arb/Acb only where their three-valued or rigorous enclosure semantics are
  explicit.
