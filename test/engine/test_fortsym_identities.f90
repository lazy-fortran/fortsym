program test_fortsym_identities
    ...
    !  Two symbols and rational coefficients. The univariate Flint cancel in
    !  the SymEngine shim used to fault on exactly this shape, taking the
    !  process with it, so the assertion here is as much "does not crash" as
    !  "is decided".
    call check("2*(y - 3*x/2)/3 - 2*y/3 + x", MUST_ZERO)
